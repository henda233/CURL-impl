"""off-policy replay buffer（环形覆盖，像素帧以 uint8 存储省内存）。

一条 transition 存：obs 帧栈 (9,84,84) uint8、action float32、reward、
next 新帧 (3,84,84) uint8。next 观测在 sample 时由 obs[3:] 与新帧拼接得到
（帧栈丢最旧帧、压入新帧），避免整份重复存储。
"""
...
class ReplayBuffer:
    def __init__(self, capacity, act_dim=6):
        ...
        self.obs = np.empty((self.capacity, STACK_C, CROP, CROP), dtype=np.uint8)
        self.next_f = np.empty((self.capacity, FRAME_C, CROP, CROP), dtype=np.uint8)
